    ...
    return products.length > 0 && cartItems ? (
        <div className="flex flex-col md:flex-row mt-16">
                ...
                {cartArray.map((product, index) => (
                    <div key={index} className="grid grid-cols-[2fr_1fr_1fr] text-gray-500 items-center text-sm md:text-base font-medium pt-3">
                        <div className="flex items-center md:gap-6 gap-3">
                            <div onClick={() => {
                                navigate(`/products/${product.cartArray.toLowerCase()}/${product._id}`); scrollTo(0, 0)
            ...
            <div className="max-w-[360px] w-full bg-gray-100/40 p-5 max-md:mt-16 border border-gray-300/70">
                <h2 className="text-xl md:text-xl font-medium">Thông tin đơn hàng</h2>
                <hr className="border-gray-300 my-5" />

                <div className="mb-6">
                    <p className="text-sm font-medium uppercase">Địa chỉ giao hàng</p>
                    <div className="relative flex justify-between items-start mt-2">
                        <p className="text-gray-500">{selectedAddress ? `${selectedAddress.street}, ${selectedAddress.city}, ${selectedAddress.state}, ${selectedAddress.country}` : "Không tìm thấy địa chỉ"}</p>
                ...
                <div className="text-gray-500 mt-4 space-y-2">
                    ...
                    <p className="flex justify-between text-lg font-medium mt-3">
                        <span>Tổng tiền thanh toán:</span><span>{currency}{getCartAmount() + getCartAmount() * 2 / 100}</span>
                    </p>
                </div>

                <button onClick={placeOrder} className="w-full py-3 mt-6 cursor-pointer bg-primary text-white font-medium hover:bg-primary-dull transition">
                    {paymenOption === "COD" ? "ĐẶT HÀNG" : "Tiến hành thanh toán"}
                </button>
            </div>
        </div>
    ) : null
